...
<body>
    ...
    <script>
        ...
        function initHostMovie(){
            ...
            $.ajax({
                ...
                success:function (obj) {
                    ...
                    //for循环遍历
                    //展示的内容包括
                    //1、obj.data[i].movie_id 电影id
                    //2、obj.data[i].movie_picture 电影海报
                    //3、obj.data[i].movie_score 电影评分
                    //4、obj.data[i].movie_cn_name 电影中国名
                    //5、注意这里虽然有两个超链接，但是javascript:void(0)是不跳转的。
                    //6、所以跳转的是buyTickets.jsp这里。
                            ...
                            "<div class=\"movie-item\">" +
                                "<a href=\"javascript:void(0)\" target=\"_blank\" data-act=\"playingMovie-click\" data-val=\"" + obj.data[i].movieId + "\">" +
                                    "<div class=\"movie-poster\" style=\"cursor:default;\">" +
                                        "<img id=\"moive_picture\" src=\"" + obj.data[i].moviePoster + "\">" +
                                        "<div class=\"movie-overlay movie-overlay-bg\">" +
                                            "<div class=\"movie-info\">" +
                                                "<div class=\"movie-score\"><i id=\"moive_score\" class=\"integer\">" + obj.data[i].movieScore + "</i></div>" +
                                                "<div class=\"movie-title movie-title-padding\" title=\"" + obj.data[i].movieName + "\">" + obj.data[i].movieName + "</div>" +
                                            "</div>" +
                                        "</div>" +
                                    "</div>" +
                                "</a>" +
                    ...
                    //即将上映展示，与上面的大同小异。
                    //最多展示8个。
                    OnNum.append("<span class=\"textcolor_blue\">即将上映（" + obj.data2.length + "部）</span>");
                    if(obj.data2.length<8){
                        ListLength = obj.data2.length;
                    }
                    else{
                        ListLength = 8;
                    }
                    ...
                    //4、obj.data[i].movie_cn_name 电影中国名
                    //5、注意这里虽然有两个超链接，但是javascript:void(0)是不跳转的。
                    //6、这里跳转的不是买座位，而是预售，预告片不会跳转（原猫眼就是这样）。
                    for(var i=0;i<ListLength;i++){
                        console.log(obj.data2[i]);
                        //随机数——用来显示多少个人想看（需要改）
                        var random = Math.floor(Math.random(1,10)*50000);
                        htmlOn =
                        "<li>" +
                            "<div class=\"movie-item\">" +
                                "<a href=\"javascript:void(0)\" target=\"_blank\" data-act=\"playingMovie-click\" data-val=\"" + obj.data2[i].movieId + "\">" +
                                    "<div class=\"movie-poster\" style=\"cursor:default;\">" +
                                        "<img id=\"moive_picture\" src=\"" + obj.data2[i].moviePoster  + "\">" +
                                        "<div class=\"movie-overlay movie-overlay-bg\">" +
                                            "<div class=\"movie-info\">" +
                                                "<div class=\"movie-score\"><i id=\"moive_score\" class=\"integer\">" + obj.data2[i].movieScore + "</i></div>" +
                                                "<div class=\"movie-title movie-title-padding\" title=\"" + obj.data2[i].movieName + "\">" + obj.data2[i].movieName + "</div>" +
                                            "</div>" +
                                        "</div>" +
                                    "</div>" +
                                "</a>" +
                    ...
                    for(var i=0;i<ListLength;i++){
                        ...
                        notice[i].onclick = function(){ 
                            layer.alert('‘' + obj.data2[this.index].movieName + '’电影暂未有预告片！',{icon: 0,offset: clientHeight/7});
                        }
                        //预售电影成功
                        sale[i].index = i;
                        sale[i].onclick = function(){
                            layer.alert('预售‘' + obj.data2[this.index].movieName + '’电影成功！',{icon: 0,offset: clientHeight/7});
                        }
                    }
                    //初始化电影票房
                    initBoxOffice(obj);
        ...
        function initBoxOffice(obj){
            ...
            //for循环遍历整个temp长度
            // i == 0 第一个电影要显示图片，不仅仅是标题。
            // id、pic、cn_name,box_office
            // i != 0 之后的就不用显示图片
            // id、cn_name,box_office
